...
import imgPhysi from "../../assets/IMG_2524.webp";
import imgAuto from "../../assets/IMG_2532.webp";

/* ── CO ROW ── */
const CoRow = ({ n, name, tag, intro, svcs, chips, clabel, src, isTab }) => (
  <div style={{ display: "grid", gridTemplateColumns: isTab ? "1fr" : "180px 1fr 240px", gap: isTab ? "16px" : "40px", padding: "44px 0", borderTop: `1px solid ${C.faint}` }}>
    ...
    {!isTab && (
      <div style={{ borderRadius: 14, overflow: "hidden", boxShadow: "0 10px 30px rgba(0,0,0,0.06)", alignSelf: "start" }}>
        <Ph src={src} label={name} height={200} />
      </div>
    )}
  </div>
);
...
const companies = [
  ...
  { n: "04", name: "Auto Pay by HoooM", tag: "Fintech", intro: "A budgeting app helping users spend wisely, save consistently, and manage money confidently. Simple tools that build healthier financial habits.", svcs: ["Expense Tracking", "Monthly Budget Setting", "Spending Visibility Dashboard", "Financial Habit Building"], chips: [], clabel: "", src: imgAuto },
];
