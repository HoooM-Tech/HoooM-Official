  ...
  return (
    <div style={{ background:C.navy, padding:"15px 0", overflow:"hidden", display:"flex" }} aria-hidden="true">
      <div className="mq-track">
        {[...items,...items].map((t,i)=>(
          <span key={i} style={{ fontFamily:"'Syne',sans-serif", fontSize:"0.76rem", fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", color: t==="✦"?C.orange:"rgba(255,255,255,.45)" }}>{t}</span>
        ))}
      </div>
    </div>
  );
