...
export const Contact = () => {
  ...
  const [state, handleSubmit] = useForm("xlgaygwp");
  ...
  return (
    <section id="contact" style={{ padding: `80px ${isTab ? "20px" : "56px"}`, background: C.off, display: "grid", gridTemplateColumns: isTab ? "1fr" : "1fr 1fr", gap: isTab ? 48 : 64, alignItems: "start" }}>
      ...
      <Reveal>
        <form ref={formRef} onSubmit={handleSubmit} style={{ background: C.white, borderRadius: 20, padding: isTab ? "24px" : "40px", boxShadow: "0 8px 36px rgba(14,19,34,.07)" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
            <div>
              <label style={lbl}>First Name</label>
              <input name="firstName" required placeholder="Peter" style={inp} onFocus={focus} onBlur={blur} />
            </div>
            <div>
              <label style={lbl}>Last Name</label>
              <input name="lastName" required placeholder="Oshinowo" style={inp} onFocus={focus} onBlur={blur} />
            </div>
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={lbl}>Email Address</label>
            <input name="email" type="email" required placeholder="your@email.com" style={inp} onFocus={focus} onBlur={blur} />
            <ValidationError prefix="Email" field="email" errors={state.errors} style={{ color: C.orange, fontSize: "0.7rem", marginTop: 4 }} />
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={lbl}>I'm interested in</label>
            <select name="interest" required style={{ ...inp, appearance: "none" }} onFocus={focus} onBlur={blur}>
              <option value="">Select a company or topic</option>
              {["Tech by HoooM — Software & Websites", "Orbbit by HoooM — Branding & Marketing", "Physifit by HoooM — Wellness Coaching", "Auto Pay — Budgeting App", "Partnership / Investment", "General Inquiry"].map(o => <option key={o} value={o}>{o}</option>)}
            </select>
          </div>
          <div style={{ marginBottom: 16 }}>
            <label style={lbl}>Message</label>
            <textarea name="message" required placeholder="Tell us about your project, goal, or question…" rows={4} style={{ ...inp, resize: "none" }} onFocus={focus} onBlur={blur} />
            <ValidationError prefix="Message" field="message" errors={state.errors} style={{ color: C.orange, fontSize: "0.7rem", marginTop: 4 }} />
          </div>
          <Btn type="submit" variant="solid" full disabled={state.submitting} style={{ background: state.succeeded ? "#1a7a3f" : undefined, opacity: state.submitting ? 0.7 : 1 }}>
            {state.submitting ? "Sending..." : state.succeeded ? "Message Sent ✓" : "Start a Conversation →"}
          </Btn>
          {state.succeeded && <p style={{ color: "#1a7a3f", fontSize: "0.8rem", marginTop: 12, textAlign: "center", fontWeight: 600 }}>Thanks! We'll get back to you shortly.</p>}
        </form>
      </Reveal>
    </section>
  );
